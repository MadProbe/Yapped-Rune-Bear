...
using Binder = SoulsFormats.Binder.Binder;
...
namespace Chomp {
    ...
    public partial class Main : Form {
        ...
        private const string ATKPARAM_NPC = "AtkParam_Npc";

        // Token: 0x0400001A RID: 26
        private const string ATKPARAM_PC = "AtkParam_Pc";

        // Token: 0x0400001D RID: 29
        private const string BEHAVIORPARAM_NPC = "BehaviorParam";

        // Token: 0x0400001C RID: 28
        private const string BEHAVIORPARAM_PC = "BehaviorParam_PC";

        // Token: 0x04000010 RID: 16
        private const int FIELD_EDITOR_NAME_COL = 1;

        // Token: 0x04000012 RID: 18
        private const int FIELD_TYPE_COL = 3;

        // Token: 0x04000011 RID: 17
        private const int FIELD_VALUE_COL = 2;
        ...
        protected internal static readonly SafeDictionary<string, SafeDictionary<string, string[]>> refs_dict = new (
             File.ReadAllText("./Paramdex/ER/defs_refs.refs")
                 .Split("\n\n")
                 .Select(static s => s.Trim().Split(':'))
                 .Select(static strings =>
        ...
        private readonly List<PARAMDEF> paramdefs = [];

        // Token: 0x04000016 RID: 22
        private readonly List<PARAMTDF> paramtdfs = [];

        // Token: 0x0400000D RID: 13
        private readonly int ROW_ID_COL;

        // Token: 0x0400000E RID: 14
        private const int ROW_NAME_COL = 1;
        ...
        private List<string> bool_type_tdfs = [];

        // Token: 0x04000019 RID: 25
        private List<string> custom_type_tdfs = [];
        ...
        public Main() {
            ...
            this.GoToReferenceMenuStripItems = [
        ...
        private void Main_Load(object sender, EventArgs e) {
            this.Text =
                $"Yapped - Rune Bear Special Edition ({Assembly.GetExecutingAssembly().GetName().Version}, {RuntimeInformation.FrameworkDescription})";
            ...
            if (settings.WindowMaximized) this.WindowState = FormWindowState.Maximized;
            this.toolStripComboBoxGame.ComboBox!.DisplayMember = "Name";
            // ReSharper disable once CoVariantArrayConversion
            this.toolStripComboBoxGame.Items.AddRange(GameMode.Modes);
            var game = Enum.Parse<GameMode.GameType>(settings.GameType);
            this.toolStripComboBoxGame.SelectedIndex = Array.FindIndex(GameMode.Modes, m => m.Game == game);
            if (this.toolStripComboBoxGame.SelectedIndex == -1) this.toolStripComboBoxGame.SelectedIndex = 0;
            if (settings.ProjectName == "") settings.ProjectName                                         = "ExampleMod";
            ...
            double timeToLoad = Tracing.TracedTiming(() => {
                Tracing.TraceTiming(this.BuildParamDefs,   nameof(this.BuildParamDefs));
                Tracing.TraceTiming(this.BuildParamTdfs,   nameof(this.BuildParamTdfs));
                Tracing.TraceTiming(this.BuildBoolTypes,   nameof(this.BuildBoolTypes));
                Tracing.TraceTiming(this.BuildCustomTypes, nameof(this.BuildCustomTypes));
                Tracing.TraceTiming(this.LoadParams,       true, nameof(this.LoadParams));
                if (this.secondaryFilePath.FileName != "") Tracing.TraceTiming(this.LoadSecondaryParams, true, nameof(this.LoadSecondaryParams));
            }, "Full param load");
            ...
            foreach (string[] components in DCVIndicesRegex().Matches(settings.DGVIndices).Select(static obj => obj.Value.Split(':')))
                this.dgvIndices[components[0]] = (int.Parse(components[1]), int.Parse(components[2]));

            if (settings.SelectedParam >= this.dgvParams.Rows.Count) settings.SelectedParam = 0;
        ...
        private void FormMain_FormClosing(object sender, FormClosingEventArgs e) {
            ...
            if (this.dgvParams.SelectedCells.Count > 0) settings.SelectedParam = this.dgvParams.SelectedCells[0].RowIndex;
            this.dgvParams.ClearSelection();
            settings.DGVIndices = string.Join(",", this.dgvIndices.Select(static kvpair => $"{kvpair.Key}:{kvpair.Value.Row}:{kvpair.Value.Cell}"));
        }

        // Token: 0x06000004 RID: 4 RVA: 0x00002704 File Offset: 0x00000904
        public string GetProjectDirectory(string subfolder) =>
            $"Projects/{settings.ProjectName}/{subfolder}/{(this.toolStripComboBoxGame.SelectedItem as GameMode).Directory}";

        // Token: 0x06000005 RID: 5 RVA: 0x00002760 File Offset: 0x00000960
        public string GetParamdexDirectory(string subfolder) =>
            $"Paramdex/{(this.toolStripComboBoxGame.SelectedItem as GameMode).Directory}{(subfolder == "" ? "" : $"/{subfolder}")}";
        ...
        private void LoadParams(bool isSilent) {
            Tracing.TraceTiming(() => {
                ...
                foreach (string name in this.primary_result.ParamWrappers.Select(static wrapper => wrapper.Name)
                                            .Where(name => !this.dgvIndices.ContainsKey(name))) this.dgvIndices[name] = default((int Row, int Cell));

                using (NamedTimings.LoadParamResultSetDataSourceNamedTiming.AsDisposable()) {
                    this.dgvParams.DataSource = this.primary_result.ParamWrappers;
                }
                foreach (DataGridViewRow row in this.dgvParams.Rows.Cast<DataGridViewRow>()
                                                    .Where(static row => (row.DataBoundItem as ParamWrapper).Error))
                    row.Cells[0].Style.BackColor = Color.Pink;
                NamedTimings.LoadParamResultSetDataSourceNamedTiming.Print();
            }, nameof(this.LoadParams));

            if (this.primary_result != null && !isSilent && !settings.ShowConfirmationMessages)
                _ = MessageBox.Show("Primary file loaded.", "File Data", MessageBoxButtons.OK);
        }
        ...
        private void LoadSecondaryParams(bool isSilent) {
            ...
            if (!isSilent && !settings.ShowConfirmationMessages) _ = MessageBox.Show("Secondary file loaded.", "File Data", MessageBoxButtons.OK);
        }

        // Token: 0x06000008 RID: 8 RVA: 0x000029B4 File Offset: 0x00000BB4
        private void BuildParamDefs() {
            this.paramdefs.Clear();
            string dirPath = this.GetParamdexDirectory("Defs");
            string cachePath = Path.Combine(dirPath, "Cache");

            if (!Directory.Exists(cachePath)) {
                Directory.CreateDirectory(cachePath);
            }

            string[] cachedFiles = Directory.GetFiles(cachePath, "*.paramdefcached");

            foreach (string path in Directory.GetFiles(dirPath, "*.xml"))
                try {
                    if (cachedFiles.FirstOrDefault(x => Path.GetFileName(x).StartsWith(Path.GetFileName(path)) &&
                                                        Path.GetFileName(x).Contains(File.GetLastWriteTime(path).ToFileTime().ToString(CultureInfo.InvariantCulture))) is { } cachedFileName) {
                        this.paramdefs.Add(PARAMDEF.Read(cachedFileName));
                    } else {
                        foreach (string redutant in cachedFiles.Where(x => Path.GetFileName(x).StartsWith(Path.GetFileName(path)))) {
                            File.Delete(redutant);
                        }
                        PARAMDEF deserialized = PARAMDEF.XmlDeserialize(path);
                        this.paramdefs.Add(deserialized);
                        deserialized.Write(Path.Combine(cachePath, $"{Path.GetFileName(path)}--{File.GetLastWriteTime(path).ToFileTime().ToString(CultureInfo.InvariantCulture)}.paramdefcached"));
        ...
        private void BuildParamTdfs() {
            ...
            foreach (string text in Directory.GetFiles(this.GetParamdexDirectory("Tdfs"), "*.tdf"))
                try {
                    var paramtdf = new PARAMTDF(File.ReadAllText(text));
                    this.paramtdfs.Add(paramtdf);
                    this.tdf_dict.Add(paramtdf.Name, paramtdf);
                } catch (Exception ex) {
                    Utility.ShowError($"""
        ...
        private void BuildBoolTypes() {
            ...
            var boolean_type_file = $"{this.GetParamdexDirectory("Meta")}/bool_types.txt";
        ...
        private void BuildCustomTypes() {
            ...
            var custom_type_file = $"{this.GetParamdexDirectory("Meta")}/customizable_types.txt";
        ...
        private LoadParamsResult LoadParamResult(string target_path, bool isSecondary) {
            ...
            var result   = new LoadParamsResult { ParamWrappers = [] };
            ...
            try {
                if (SoulsFile<BND4>.Is(target_path)) {
                    ...
                    switch (gameMode.Game) {
                        case GameMode.GameType.DarkSouls2 or GameMode.GameType.DarkSouls2Scholar:
            ...
            if (!isSecondary) this.processLabel.Text = target_path;
            Tracing.TraceTiming(() => {
                foreach (BinderFile file in result.ParamBND.Files.Where(static f => f.Name.EndsWith(".param"))) {
                    string name = Path.GetFileNameWithoutExtension(file.Name);

                    try {
                        PARAM param;
                        using (NamedTimings.LoadParamResultSoulsFilePARAMReadNamedTiming.AsDisposable()) {
                            param = SoulsFile<PARAM>.Read(file.Bytes);
                        }
                        using (NamedTimings.LoadParamResultApplyParamdefNamedTiming.AsDisposable()) {
                            if (param.ApplyParamdefCarefully(this.paramdefs)) {
                                result.ParamWrappers.Add(new ParamWrapper(name, param, file));
                                file.Bytes = null;
                            }
                        }
                    } catch (Exception ex2) {
            ...
            }, nameof(this.LoadParamResult));
            NamedTimings.LoadParamResultSoulsFilePARAMReadNamedTiming.Print();
            NamedTimings.LoadParamResultApplyParamdefNamedTiming.Print();
        ...
        private void toggleFieldNameTypeToolStripMenuItem_Click(object sender, EventArgs e) {
            if (this.InvalidationMode) return;
        ...
        private void OpenToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (this.dataFileDialog.ShowDialog() != DialogResult.OK) return;
        ...
        private void SaveToolStripMenuItem_Click(object sender, EventArgs e) {
            if (this.InvalidationMode) return;
            Tracing.TraceTiming(this.SaveParams, ".bak", nameof(this.SaveToolStripMenuItem_Click));
            if (!settings.ShowConfirmationMessages) _ = MessageBox.Show($"Params saved to {this.regulationPath}", "Save", MessageBoxButtons.OK);
        }

        // Token: 0x06000013 RID: 19 RVA: 0x0000316C File Offset: 0x0000136C
        private void SaveParams(string backup_format = ".bak") {
            foreach (ParamWrapper paramFile in this.primary_result.ParamWrappers)
                try {
                    paramFile.BinderFile.Bytes      = paramFile.Param.Write();
                } catch {
                    _ = MessageBox.Show($"Invalid data, failed to save {paramFile}. Data must be fixed before saving can complete.", "Save",
                                        MessageBoxButtons.OK);
                    return;
                }

            var gameMode = this.toolStripComboBoxGame.SelectedItem as GameMode;
            if (!File.Exists(this.regulationPath + backup_format)) File.Copy(this.regulationPath, this.regulationPath + backup_format);
            ...
            foreach (ParamWrapper paramFile in this.primary_result.ParamWrappers)
                paramFile.BinderFile.Bytes = null;
        ...
        private void RestoreToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (File.Exists($"{this.regulationPath}.bak")) {
                if (MessageBox.Show("Are you sure you want to restore the backup?", "Confirmation", MessageBoxButtons.YesNo,
        ...
        private void ExportToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (this.fbdExport.ShowDialog() != DialogResult.OK) return;
            ...
            try {
                new BND4 {
                    ...
                    Unk04       = false,
                    Unk05       = false,
                    Format      = Binder.Format.Names1 | Binder.Format.LongOffsets | Binder.Format.Compression | Binder.Format.Flag6,
                    Unicode     = true,
                    Files       = this.regulation.Files.Where(static f => f.Name.EndsWith(".param")).ToList(),
                ...
                if ((this.toolStripComboBoxGame.SelectedItem as GameMode).Game == GameMode.GameType.DarkSouls3)
                    new BND4 {
                        ...
                        Format      = Binder.Format.Names1 | Binder.Format.LongOffsets | Binder.Format.Compression | Binder.Format.Flag6,
                        Unicode     = true,
                        Files       = this.regulation.Files.Where(static f => f.Name.EndsWith(".stayparam")).ToList(),
        ...
        private void DuplicateRowToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (newRowForm.ShowDialog() != DialogResult.OK) return;
            ...
            ParamWrapper paramWrapper = this.rowSource.DataSource as ParamWrapper ?? Unsafe.NullRef<ParamWrapper>();
            Row newRow;
            ...
            for (var i = 0; i < repeat_count; i++) {
                if (paramWrapper.Rows.Any(row => row.ID == current_id)) {
                    ...
                    newRow = new Row(current_id, name, paramWrapper.Param);
                    _      = this.rowSource.Add(newRow);
                    List<Row> rows2 = paramWrapper.Rows;
                    int row_index = rows2.FindIndex(row => row == newRow);
                    ...
                    for (var j = 0; j < oldRow.Cells.Count; j++) newRow.Cells[j].Value = oldRow.Cells[j].Value;
                }

                current_id += step_value;
            }

            paramWrapper.Rows.Sort(static (r1, r2) => r1.ID - r2.ID);
        }
        ...
        private Row CreateRow(string prompt) {
            ...
            if (newRowForm.ShowDialog() != DialogResult.OK) return row_result;
            ...
            if (paramWrapper.Rows.Any(row => row.ID == id)) {
                ...
                row_result = new Row(id, name, paramWrapper.Param);
                _          = this.rowSource.Add(row_result);
                paramWrapper.Rows.Sort(static (r1, r2) => r1.ID.CompareTo(r2.ID));
                int index = paramWrapper.Rows.FindIndex(row => row == row_result);
                int displayedRows = this.dgvRows.DisplayedRowCount(false);
        ...
        private void DeleteRowToolStripMenuItem_Click(object sender, EventArgs e) {
            if (this.InvalidationMode || this.dgvRows.SelectedCells.Count <= 0) return;
            var choice = DialogResult.Yes;
            if (settings.VerifyRowDeletion)
                choice = MessageBox.Show("Are you sure you want to delete this row?", "Confirmation", MessageBoxButtons.YesNo,
                                         MessageBoxIcon.Question);
            if (choice != DialogResult.Yes) return;
            int rowIndex = this.dgvRows.SelectedCells[0].RowIndex;
            this.rowSource.RemoveAt(rowIndex);
            if (rowIndex != this.dgvRows.RowCount) return;
        ...
        private void FindRowToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (findForm.ShowDialog() == DialogResult.OK) this.FindRow(findForm.ResultPattern);
        }
        ...
        private void FindNextRowToolStripMenuItem_Click(object sender, EventArgs e) {
            if (!this.InvalidationMode) this.FindRow(this.lastFindRowPattern);
        }
        ...
        private void GotoRowToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            int index = (this.rowSource.DataSource as ParamWrapper).Rows.FindIndex(row => row.ID == id);
        ...
        private void FindFieldToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (findForm.ShowDialog() == DialogResult.OK) this.FindField(findForm.ResultPattern);
        }
        ...
        private void FindField(string pattern) {
            ...
            if (this.dgvCells.DataSource != null) {
                ...
                var cells = this.dgvCells.DataSource as ICell[];
                int index = -1;
                ...
                for (var i = 0; i < length; i++)
                    if (settings.CellView_ShowEditorNames) {
                        if (!(cells[(startIndex + i) % length].EditorName ?? "").ToLower().Contains(value)) continue;
                        index = (startIndex + i) % length;
                        break;
                    } else if ((cells[(startIndex + i) % length].Name ?? "").ToLower().Contains(pattern)) {
                        index = (startIndex + i) % length;
                        break;
                    }
        ...
        private void importRowNames_Project_MenuItem_Click(object sender, EventArgs e) {
            ...
            foreach (ParamWrapper paramFile in (List<ParamWrapper>)this.dgvParams.DataSource)
                if (File.Exists($"{name_dir}\\{paramFile.Name}.txt")) {
                    ...
                    foreach (Row row in paramFile.Param.Rows.Where(row => names.ContainsKey(row.ID) && (replace || row.Name is null or "")))
        ...
        private void importRowNames_Stock_MenuItem_Click(object sender, EventArgs e) {
            ...
            foreach (ParamWrapper paramFile in (List<ParamWrapper>)this.dgvParams.DataSource)
                if (File.Exists($"{name_dir}\\{paramFile.Name}.txt")) {
                    ...
                    foreach (Row row in paramFile.Param.Rows.Where(row => names.ContainsKey(row.ID) &&
                                                                          (replace || row.Name == null || row.Name == "")))
                        row.Name = names[row.ID];
                }
        ...
        private void exportRowNames_Project_MenuItem_Click(object sender, EventArgs e) {
            ...
            foreach (ParamWrapper paramFile in this.dgvParams.DataSource as List<ParamWrapper>) {
                ...
                foreach (Row row in paramFile.Param.Rows) {
                    ...
                    if (name != "") _ = sb.AppendLine($"{row.ID} {name}");
                }
            ...
            if (!settings.ShowConfirmationMessages) _ = MessageBox.Show("Names exported!", "Export Names", MessageBoxButtons.OK);
        }
        ...
        private void exportDataMenuItem_Click(object sender, EventArgs e) {
            ...
            Tracing.PrintTiming("FileWriter.encoder.GetBytes");
            Tracing.PrintTiming("FileWriter.Write");
        }
        ...
        private void fieldExporterMenuItem_Click(object sender, EventArgs e) {
            if (this.InvalidationMode) return;
            if (new FieldExporter().ShowDialog() != DialogResult.OK) return;
            ParamWrapper paramWrapper = this.rowSource.DataSource as ParamWrapper ?? Unsafe.NullRef<ParamWrapper>();
            var field_data_path = $"{this.GetProjectDirectory("Logs")}\\\\FieldValue_{paramWrapper.Name}.log";
            ...
            if (File.Exists(field_data_path)) {
                if (MessageBox.Show($"{field_data_path} exists. Overwrite?", "Export Field Values", MessageBoxButtons.YesNo,
            ...
            if (settings.IncludeHeaderInCSV) output_file.WriteLine($"Row ID{exportDelimiter}Row Name{exportDelimiter}{field_match}");
            var unique_list = new List<string>();

            foreach (Row row in paramWrapper.Rows) {
                var row_line = $"{row.ID}{exportDelimiter}{(settings.IncludeRowNameInCSV ? row.Name + exportDelimiter : "")}";
                ...
                foreach (ICell cell in row.Cells) {
                    ...
                    if (type == PARAMDEF.DefType.dummy8 ||
                        ...
                        (field_minimum != "" && !(type switch {
                            ...
                            _ => true,
                        })) || (settings.ExportUniqueOnly && !unique_list.AddNoReplacement(value))) continue;
                    isValidRow = true;
                    row_line += type switch {
                        ...
                        PARAMDEF.DefType.f32 => Convert.ToSingle(value).ToString(CultureInfo.InvariantCulture),
                        PARAMDEF.DefType.fixstr or PARAMDEF.DefType.fixstrW => value,
                        _ => "",
                    };
                }

                if (isValidRow) output_file.WriteLine(row_line);
            }
        ...
        private void rowReferenceFinderMenuItem_Click(object sender, EventArgs e) {
            ...
            if (newFormReferenceFinder.ShowDialog() == DialogResult.OK) {
                ...
                var reference_file_path = $"{this.GetProjectDirectory("Logs")}\\\\RowReference.log";
                ...
                foreach (ParamWrapper param in this.primary_result.ParamWrappers) {
                    foreach (Row row in param.Rows) {
                        foreach (ICell cell in row.Cells) {
                            ...
                            if (threw || !CheckFieldReference(reference_text, value_2)) continue;
                            output_file.WriteLine(param.Name);
                            if (row.Name != null) output_file.WriteLine($"  - Row Name: {row.Name}");
                            if (cell.Def?.ToString() != null) output_file.WriteLine($"  - Cell Name: {cell.Def}");
        ...
        private void valueReferenceFinderMenuItem_Click(object sender, EventArgs e) {
            ...
            if (newFormReferenceFinder.ShowDialog() != DialogResult.OK) return;
            string reference_text = newFormReferenceFinder.GetReferenceText();
            ...
            var reference_file_path = $"{this.GetProjectDirectory("Logs")}/ValueReference.log";

            using (var output_file = new StreamWriter(reference_file_path)) {
                foreach (ParamWrapper param in this.primary_result.ParamWrappers) {
                    foreach (Row row in param.Rows.Where(row => CheckFieldReference(reference_text, row.ID))) {
                        output_file.WriteLine(param.Name);
                        if (row.Name != null) output_file.WriteLine($"  - Row Name: {row.Name}");
                        output_file.WriteLine($"  - Row ID: {row.ID}");
                        output_file.WriteLine($"  - Reference Value: {reference_text}");
        ...
        private static bool CheckFieldReference(string value_1, string value_2) =>
            int.TryParse(value_1, out int v1) && int.TryParse(value_2, out int v2) && v1 == v2;
        ...
        private void massImportDataMenuItem_Click(object sender, EventArgs e) =>
            _ = this.InvalidationMode
                || MessageBox
                    .Show("Mass Import will import data from CSV files for all params. Continue?",
                          "Mass Import", MessageBoxButtons.YesNo,
                          MessageBoxIcon.Exclamation) == DialogResult.No
                || this.primary_result == null ||
                MessageBox
                    .Show($"Mass data import complete in {Tracing.TracedTiming(delegate {
                        this.primary_result.ParamWrappers.ForEach(wrapper => this.ImportParamData(wrapper));
                    }, nameof(this.massImportDataMenuItem_Click))} ms!", "Mass Import") <= DialogResult.None;
        ...
        private void massExportDataMenuItem_Click(object sender, EventArgs e) =>
            _ = this.InvalidationMode ||
                MessageBox
                    .Show("Mass Export will export all params to CSV. Continue?",
                          "Mass Export", MessageBoxButtons.YesNo,
                          MessageBoxIcon.Exclamation) == DialogResult.No ||
                (!settings.IncludeRowNameInCSV &&
                 MessageBox
                     .Show("Row Names are currently not included. Continue?",
                           "Mass Export", MessageBoxButtons.YesNo,
                           MessageBoxIcon.Exclamation) == DialogResult.No)
                || (this.primary_result != null &&
                    MessageBox
                        .Show($"Mass data export complete in {Tracing.TracedTiming(() => {
                            this.primary_result.ParamWrappers.ForEach(wrapper => this.ExportParamData(wrapper, true));

                            NamedTimings.FileWriterFlushEncodeNamedTiming.Print();
                            NamedTimings.FileWriterFlushFileWriteNamedTiming.Print();
                        }, nameof(this.massExportDataMenuItem_Click))} ms!",
                              "Data Export") > DialogResult.None);
        ...
        private void fieldAdjusterMenuItem_Click(object sender, EventArgs e) {
            if (this.InvalidationMode || new FieldAdjuster().ShowDialog() != DialogResult.OK) return;
            this.SaveParams();
            ParamWrapper paramWrapper = this.rowSource.DataSource as ParamWrapper ?? Unsafe.NullRef<ParamWrapper>();
            var adjustment_file_path = $"{this.GetProjectDirectory("Logs")}/FieldAdjustment_{paramWrapper.Name}.log";
            char delimiter = settings.ExportDelimiter.ToCharArray()[0];
            ...
            var row_range_array_0 = 0;
            var row_range_array_1 = int.MaxValue;
            _ = NumberRegex().Matches(row_range_array[0]).Count == 1 &&
                int.TryParse(row_range_array[0], out row_range_array_0);
            _ = NumberRegex().Matches(row_range_array[1]).Count == 1 &&
                int.TryParse(row_range_array[1], out row_range_array_1);
            bool show_editor_names = settings.CellView_ShowEditorNames;
            ...
            using (var output_file = new StreamWriter(adjustment_file_path)) {
                foreach (Row row in paramWrapper.Rows)
                    if ((row_range == "" || (row.ID >= row_range_array_0 && row.ID <= row_range_array_1)) &&
                        (row_partial_match == "" || row.ID.ToString()[^row_partial_match.Length..] == row_partial_match))
                        foreach (ICell cell in row.Cells) {
                            PARAMDEF.DefType type = cell.Def.DisplayType;
                            var value = cell.Value.ToString();
                            ...
                            output_file.WriteLine($"- Field {cell.Def.InternalName}");
                            output_file.WriteLine($"- Old Value {cell.Value}");
                            ...
                            if (field_formula.Contains('x')) {
                                ...
                                if (output_max != "" && field_result > decimal.Parse(output_max)) field_result = decimal.Parse(output_max);
                                if (output_min != "" && field_result < decimal.Parse(output_min)) field_result = decimal.Parse(output_min);
                            } else {
                                field_result = decimal.Parse(field_formula);
                            }

                            cell.Value = type switch {
                                PARAMDEF.DefType.s8 => Convert.ToSByte(field_result),
                                PARAMDEF.DefType.u8 => Convert.ToByte(field_result),
                                PARAMDEF.DefType.s16 => Convert.ToInt16(field_result),
                                PARAMDEF.DefType.u16 => Convert.ToUInt16(field_result),
                                PARAMDEF.DefType.s32 => Convert.ToInt32(field_result),
                                PARAMDEF.DefType.u32 => Convert.ToUInt32(field_result),
                                PARAMDEF.DefType.f32 => Convert.ToSingle(field_result),
                                _ => cell.Value,
                            };
                            output_file.WriteLine($"- New Value {cell.Value}");
                            output_file.WriteLine("");
                            // }
                        }
            }
            ...
            if (!settings.ShowConfirmationMessages) _ = MessageBox.Show("Field Adjustment complete.", "Field Adjuster", MessageBoxButtons.OK);
        }

        [DoesNotReturn]
        private T ThrowCannotGetParamWrapper<T>() =>
            throw new NullReferenceException($"{nameof(this.rowSource.DataSource)} Row source has null DataSource");

        private static void StartTextEditorIfNecessary(string file_path) {
            if (!settings.UseTextEditor || settings.TextEditorPath == "") return;
        ...
        private void affinityGeneratorMenuItem_Click(object sender, EventArgs e) {
            ...
            AffinityGeneration.GenerateAffinityRows(currentRow, currentRow.DataBoundItem as ParamWrapper, this.dgvRows,
                                                    this.toolStripComboBoxGame.SelectedItem as GameMode);
        }

        // Token: 0x06000030 RID: 48 RVA: 0x0000661C File Offset: 0x0000481C
        [SkipLocalsInit]
        private unsafe void ImportParamData(ParamWrapper wrapper, bool isSilent = true) {
            string paramName = wrapper.Name;
            var paramPath = $"{this.GetProjectDirectory("CSV")}\\{paramName}.csv";

            if (!File.Exists(paramPath)) {
                if (!isSilent) _ = MessageBox.Show($"{paramPath} does not exist.", "Import Data");
                return;
            }

            if (!isSilent && MessageBox.Show($"Importing will overwrite {paramName} data. Continue?", "Import Data", MessageBoxButtons.YesNo,
                                             MessageBoxIcon.Exclamation) == DialogResult.No) return;
            StreamReader reader;
            ...
            using (reader) {
                ...
                var rows_length = (ulong)(rows.Count * Unsafe.SizeOf<Row>() + FieldBitOffsetMap.ARRAY_TO_CONTENTS_POINTER_OFFSET);
                ...
                Allocator allocator = wrapper.Param.MiniCellsAllocator;
                FieldBitOffsetMap fieldBitOffsetMap = wrapper.AppliedParamDef.FieldBitOffsetMap;
                PARAMDEF.DefType* defTypesFilteredRef = fieldBitOffsetMap.DefTypesFilteredRef;
                int* offsetsFilteredRef = fieldBitOffsetMap.OffsetsFilteredRef;
                int* fieldOffsetsFilteredRef = fieldBitOffsetMap.FieldOffsetsFilteredRef;
                int* arrayLengthsRef = fieldBitOffsetMap.ArrayLengthsFilteredRef;
                int cells_filtered_length = fieldBitOffsetMap.DefTypesFiltered.Length;
                PARAMDEF.Field[] fields = wrapper.AppliedParamDef.Fields.AsContents();

                while (!reader.EndOfStream) {
                    string line = reader.ReadLine();

                    fixed (char* p = line, export_delimeter_pointer = export_delimeter) {
                        var splitter = new IterativeStringSplitterSmartVectorized(new ReadOnlySpan<char>(p, line.Length), export_delimeter_pointer,
                                                                                  export_delimeter_length);
                        ...
                        if (rows_length > i) {
                            ...
                            newRow = rows.EchoAdd(new Row(id, includeRowNameInCSV ? splitter.NextString() : string.Empty, wrapper.Param));
                        }
                        ...
                        //     }
                        // }
                        MiniCell* miniCells = newRow.MiniCells.GetOffset(allocator);

                        for (var j = 0; j < cells_filtered_length; j++) {
                            ReadOnlySpan<char> chars = splitter.Next();

                            try {
                                miniCells.AtRef(offsetsFilteredRef[j]).SetValue(defTypesFilteredRef[j], chars, arrayLengthsRef[j]);
                                // if (type == PARAMDEF.DefType.u8) {
                                //     cell.Value = Convert.ToByte(values[cell_index], CultureInfo.InvariantCulture);
                                // }
                                // if (type == PARAMDEF.DefType.s16) {
                                ...
                                PARAMDEF.Field field = fields.At(fieldOffsetsFilteredRef[j]);
                                Utility.ShowError($"Row {newRow.ID}, Field {field.InternalName} of {field.Def.ParamType}, {field.DisplayType} and {field.DisplayName} has invalid value \"{chars}\", skipped import of this value.",
                                                  "Data Import");
                            }
                        }
                    }
                ...
                if (i < rows_length) {
                    ...
                    for (; i != rows_length; i += 8) rows_array.AssignAnyAt(i, 0UL);
                } else if (i > rows_length) {
                    rows.Sort(static (r1, r2) => r1.ID - r2.ID);
                }
            }

            if (!settings.ShowConfirmationMessages && !isSilent) _ = MessageBox.Show($"{paramName} data import complete!", "Data Import");
        }
        ...
        private unsafe void ExportParamData(ParamWrapper wrapper, bool isSilent) {
            bool includeRowNameInCSV = settings.IncludeRowNameInCSV;
            ReadOnlySpan<char> export_delimeter_chars = settings.ExportDelimiter;
            var paramPath = $"{this.GetProjectDirectory("CSV")}/{wrapper.Name}.csv";
            List<Row> rows = wrapper.Rows;
            if (!isSilent && File.Exists(paramPath) &&
                MessageBox.Show($"{paramPath} exists. Overwrite?", "Export Data", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) ==
                DialogResult.No) return;
            _ = Directory.CreateDirectory(this.GetProjectDirectory("CSV"));
            {
                using var out_file = new FileWriter(paramPath, fileStreamOptions, stackalloc char[0x4000]);
                Allocator allocator = wrapper.Param.MiniCellsAllocator;
                FieldBitOffsetMap fieldBitOffsetMap = wrapper.AppliedParamDef.FieldBitOffsetMap;
                PARAMDEF.DefType* defTypesFilteredRef = fieldBitOffsetMap.DefTypesFilteredRef;
                int* offsetsFilteredRef = fieldBitOffsetMap.OffsetsFilteredRef;
                ...
                if (cells_filtered_length != 0) {
                    if (settings.VerboseCSVExport) {
                        ...
                        for (ref Row rowref = ref MemoryMarshal.GetArrayDataReference(rows_array),
                                     endref = ref Unsafe.Add(ref rowref, rows.Count);
                             Unsafe.IsAddressLessThan(ref rowref, ref endref);
                             rowref = ref Unsafe.Add(ref rowref, 1)) {
                            Row row = rowref;
                            ...
                            out_file.WriteSmallString(export_delimeter_chars);
                            MiniCell* cells = row.MiniCells.GetOffset(allocator);

                            for (var start = 0; start < cells_filtered_prelast_length; start++) {
                                out_file.WriteSmallString("~#");
                                out_file.Write(defTypesFilteredRef[start], ref cells.AtRef(offsetsFilteredRef[start]), arrayLengthsRef[start]);
                                out_file.WriteSmallString(export_delimeter_chars);
                            }

                            out_file.WriteSmallString("~#");
                            out_file.WriteLine(defTypesFilteredRef[cells_filtered_prelast_length],
                                               ref cells.AtRef(offsetsFilteredRef[cells_filtered_prelast_length]),
                                               arrayLengthsRef[cells_filtered_prelast_length]);
                        ...
                        if (settings.IncludeHeaderInCSV) {
                            ...
                            for (var i = 0; i < cells_filtered_prelast_length; i++) {
                                out_file.Write(fields.At(fieldOffsetsFilteredRef[i]).InternalName);
                                out_file.WriteSmallString(export_delimeter_chars);
                            }

                            out_file.WriteLine(fields.At(fieldOffsetsFilteredRef[cells_filtered_prelast_length]).InternalName);
                        }
                        for (ref Row rowref = ref MemoryMarshal.GetArrayDataReference(rows_array),
                                     endref = ref Unsafe.Add(ref rowref, rows.Count);
                             Unsafe.IsAddressLessThan(ref rowref, ref endref);
                            ...
                            ref MiniCell cells = ref row.MiniCells.GetOffsetRef(wrapper.Param.MiniCellsAllocator);

                            for (var start = 0; start < cells_filtered_prelast_length; start++) {
                                out_file.Write(defTypesFilteredRef[start], ref Unsafe.AddByteOffset(ref cells, offsetsFilteredRef[start]),
                                               arrayLengthsRef[start]);
                                out_file.WriteSmallString(export_delimeter_chars);
                            }

                            out_file.WriteLine(defTypesFilteredRef[cells_filtered_prelast_length],
                                               ref Unsafe.AddByteOffset(ref cells, offsetsFilteredRef[cells_filtered_prelast_length]),
                                               arrayLengthsRef[cells_filtered_prelast_length]);
                            // for (void** i2 = Pointers.ManagedToPointerArray(cells) + sizeof(ulong), end = i2 + *(ulong*)Pointers.ManagedToPointer(cells); i2 != end;) {
                            //     PARAM.Cell cell = Pointers.PointerToManaged<PARAM.Cell>(*i2);
                            //     if (cell.Def.DisplayType != PARAMDEF.DefType.dummy8) {
            ...
            if (!isSilent && !settings.ShowConfirmationMessages) _ = MessageBox.Show($"{paramPath} data export complete!", "Data Export");
        }
        ...
        private void DgvParams_SelectionChanged(object sender, EventArgs e) {
            if (this.rowSource.DataSource != null)
                this.dgvIndices[(this.rowSource.DataSource as ParamWrapper ?? Unsafe.NullRef<ParamWrapper>()).Name] = (
            ...
            this.dgvCells.DataSource  = null;
            if (this.dgvParams.SelectedCells.Count <= 0) return;
            settings.SelectedParam = this.dgvParams.SelectedCells[0].RowIndex;
            ParamWrapper paramFile2 = this.dgvParams.SelectedCells[0].OwningRow.DataBoundItem as ParamWrapper ?? Unsafe.NullRef<ParamWrapper>();
            ...
            if (indices2.Row >= this.dgvRows.RowCount) indices2.Row = this.dgvRows.RowCount - 1;
            if (indices2.Row < 0) indices2.Row                      = 0;
            this.dgvIndices[paramFile2.Name] = indices2;
            this.dgvRows.ClearSelection();
            if (this.dgvRows.RowCount <= 0) return;
        ...
        private void DgvParams_CellToolTipTextNeeded(object sender, DataGridViewCellToolTipTextNeededEventArgs e) {
            ...
            ParamWrapper paramWrapper = this.dgvParams.Rows[e.RowIndex].DataBoundItem as ParamWrapper ?? Unsafe.NullRef<ParamWrapper>();
        ...
        private void DgvRows_SelectionChanged(object sender, EventArgs e) {
            if (this.dgvRows.SelectedCells.Count <= 0) return;
            settings.SelectedRow = this.dgvRows.SelectedCells[0].RowIndex;
            ParamWrapper paramFile = this.dgvParams.SelectedCells[0].OwningRow.DataBoundItem as ParamWrapper ?? Unsafe.NullRef<ParamWrapper>();
            (int Row, int Cell) indices = this.dgvIndices[paramFile.Name];
            if (this.dgvCells.FirstDisplayedScrollingRowIndex >= 0) indices.Cell = this.dgvCells.FirstDisplayedScrollingRowIndex;
            var row = (Row)this.dgvRows.SelectedCells[0].OwningRow.DataBoundItem;
            this.dgvCells.DataSource = row.Cells.Where(static cell => cell.Def.DisplayType != PARAMDEF.DefType.dummy8).ToArray();
            if (indices.Cell >= this.dgvCells.RowCount) indices.Cell = this.dgvCells.RowCount - 1;
            if (indices.Cell < 0) indices.Cell                       = 0;
            this.dgvIndices[paramFile.Name] = indices;
            if (this.dgvCells.RowCount > 0) this.dgvCells.FirstDisplayedScrollingRowIndex = indices.Cell;

            for (var i = 0; i < this.dgvCells.Rows.Count; i++) {
                var cell3 = this.dgvCells.Rows[i].DataBoundItem as ICell;
                if (!settings.ShowEnums || settings.ParamDifferenceMode ||
                    !this.tdf_dict.TryGetValue(cell3.Def.InternalType, out PARAMTDF tdf)) continue;
                ...
                if (!enum_dict.ContainsKey(cell3.Value)) continue;

                if (this.bool_type_tdfs.Contains(tdf.Name) && settings.DisplayBooleanEnumAsCheckbox) {
                    this.dgvCells.Rows[i].Cells[FIELD_VALUE_COL] = new DataGridViewCheckBoxCell {
                        TrueValue  = "1",
                        FalseValue = "0",
                    ...
                    if (!this.custom_type_tdfs.Contains(tdf.Name))
                        (this.dgvCells.Rows[i].Cells[FIELD_VALUE_COL] = new DataGridViewComboBoxCell {
            ...
            Color int_color = Color.FromArgb(settings.FieldColor_Int_R,       settings.FieldColor_Int_G,    settings.FieldColor_Int_B);
            Color float_color = Color.FromArgb(settings.FieldColor_Float_R,   settings.FieldColor_Float_G,  settings.FieldColor_Float_B);
            Color bool_color = Color.FromArgb(settings.FieldColor_Bool_R,     settings.FieldColor_Bool_G,   settings.FieldColor_Bool_B);
            Color string_color = Color.FromArgb(settings.FieldColor_String_R, settings.FieldColor_String_G, settings.FieldColor_String_B);

            for (var j = 0; j < this.dgvCells.Rows.Count; j++) {
                DataGridViewRow cell2 = this.dgvCells.Rows[j];
                var type = cell2.Cells[FIELD_TYPE_COL].Value.ToString();
                cell2.Cells[2].Style.BackColor = this.darkTheme ? GridBackColor : Color.White;

                switch (settings.ParamDifferenceMode) {
                    case false:
                        cell2.Cells[2].Style.BackColor = type.Contains("BOOL") || type.Contains("ON_OFF")
                            ? bool_color
                            : type == "f32"
                                ? float_color
                                : type is "fixStr" or "fixStrW"
                                    ? string_color // : type is "u32" or "s32" or "u16" or "s16" or "u8" or "s8"
                                    // ? int_color
                                    : int_color;
                        break;
                    case true when this.secondary_result != null: {
                        foreach (Row secondary_row in this.secondary_result.ParamWrappers
                            ...
                            for (var p = 0; p < secondary_row.Cells.Count; p++) {
                                ICell primary_cell = row.Cells[p];
                                ICell secondary_cell = secondary_row.Cells[p];
        ...
        private void DgvRows_CellValidating(object sender, DataGridViewCellValidatingEventArgs e) {
            if (e.ColumnIndex == this.ROW_ID_COL &&
                ((!int.TryParse((string)e.FormattedValue, out int value) && (string)e.FormattedValue != "") || value < 0)) {
                Utility.ShowError("""
        ...
        private void dgvRows_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e) {
            if (this.InvalidationMode) return;
            if (e.ColumnIndex == -1 || e.RowIndex == -1 || e.Button != MouseButtons.Right || sender is not DataGridView dgv) return;
        ...
        private void dgvRows_CellContextMenuStripNeeded(object sender, DataGridViewCellContextMenuStripNeededEventArgs e) {
            ...
            var row_param_name = this.dgvParams.CurrentCell.Value.ToString();
            if (row_param_name == ATKPARAM_PC || row_param_name == ATKPARAM_NPC || row_param_name == BEHAVIORPARAM_PC ||
                row_param_name == BEHAVIORPARAM_NPC) e.ContextMenuStrip = this.rowContextMenu;
        }
        ...
        private void rowContextMenu_Opening(object sender, CancelEventArgs e) {
            ...
            switch (row_param_name) {
                case ATKPARAM_PC:
                case ATKPARAM_NPC:
                case BEHAVIORPARAM_PC:
                case BEHAVIORPARAM_NPC:
                    this.copyToParamMenuItem.Visible = true;
                    this.copyToParamMenuItem.Text = $"Copy {row.ID} to {true switch {
                        true when row_param_name == ATKPARAM_PC => ATKPARAM_NPC,
                        true when row_param_name == ATKPARAM_NPC => ATKPARAM_PC,
                        true when row_param_name == BEHAVIORPARAM_PC => BEHAVIORPARAM_NPC,
                        true when row_param_name == BEHAVIORPARAM_NPC => BEHAVIORPARAM_PC,
        ...
        private void copyToParamMenuItem_Click(object sender, EventArgs e) {
            ...
            var current_param_name = this.dgvParams.CurrentCell.Value.ToString();
            string target_param_name = true switch {
                true when current_param_name == ATKPARAM_PC => ATKPARAM_NPC,
                true when current_param_name == ATKPARAM_NPC => ATKPARAM_PC,
                true when current_param_name == BEHAVIORPARAM_PC => BEHAVIORPARAM_NPC,
                true when current_param_name == BEHAVIORPARAM_NPC => BEHAVIORPARAM_PC,
                _ => "",
            };
            ParamWrapper target_wrapper = this.primary_result.ParamWrappers.FirstOrDefault(wrapper => wrapper.Name == target_param_name);
            ...
            if (target_wrapper.Rows.Any(_ => row.ID == id)) {
                Utility.ShowError($"A row with this ID already exists: {id}");
                return;
            }

            var row_result = new Row(id, name, target_wrapper.Param);
            for (var i = 0; i < row.Cells.Count; i++) row_result.Cells[i].Value = row.Cells[i].Value;
            target_wrapper.Rows.Add(row_result);
            target_wrapper.Rows.Sort(static (r1, r2) => r1.ID - r2.ID);

            for (var j = 0; j < this.dgvParams.Rows.Count; j++)
                if (this.dgvParams.Rows[j].Cells[0].Value.ToString() == target_param_name) {
                    this.dgvParams.ClearSelection();
                    this.dgvParams.Rows[j].Selected = true;
                }

            for (var k = 0; k < this.dgvRows.Rows.Count; k++)
                if (id == Convert.ToInt32(this.dgvRows.Rows[k].Cells[0].Value)) {
                    this.dgvRows.ClearSelection();
                    this.dgvRows.Rows[k].Selected = true;
                    this.dgvRows.CurrentCell      = this.dgvRows.Rows[k].Cells[0];
                }
        }
        ...
        private void dgvCells_SelectionChanged(object sender, EventArgs e) {
            if (this.dgvCells.SelectedCells.Count > 0) settings.SelectedField = this.dgvCells.SelectedCells[0].RowIndex;
        }
        ...
        private void DgvCells_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e) {
            if (settings.CellView_ShowEditorNames) {
                ...
                this.dgvCells.Columns[FIELD_EDITOR_NAME_COL].Visible     = true;
            } else {
                this.dgvCells.Columns[this.FIELD_PARAM_NAME_COL].Visible = true;
                this.dgvCells.Columns[FIELD_EDITOR_NAME_COL].Visible     = false;
            }

            if (settings.CellView_ShowTypes) {
                this.dgvCells.Columns[FIELD_TYPE_COL].Visible       = true;
                this.dgvCells.Columns[FIELD_TYPE_COL].AutoSizeMode  = DataGridViewAutoSizeColumnMode.Fill;
                this.dgvCells.Columns[FIELD_VALUE_COL].AutoSizeMode = DataGridViewAutoSizeColumnMode.AllCells;
                return;
            }

            this.dgvCells.Columns[FIELD_TYPE_COL].Visible       = false;
            this.dgvCells.Columns[FIELD_TYPE_COL].AutoSizeMode  = DataGridViewAutoSizeColumnMode.AllCells;
            this.dgvCells.Columns[FIELD_VALUE_COL].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
        }
        ...
        private void DgvCells_CellValidating(object sender, DataGridViewCellValidatingEventArgs e) {
            if (!settings.EnableFieldValidation || e.ColumnIndex != FIELD_VALUE_COL) return;
            var cell = this.dgvCells.Rows[e.RowIndex].DataBoundItem as ICell;
            if (cell.Def.DisplayType == PARAMDEF.DefType.fixstr || cell.Def.DisplayType == PARAMDEF.DefType.fixstrW ||
                (settings.ShowEnums && this.tdf_dict.ContainsKey(cell.Def.InternalType))) return;
        ...
        private void DgvCells_CellToolTipTextNeeded(object sender, DataGridViewCellToolTipTextNeededEventArgs e) {
            if ((e.ColumnIndex == this.FIELD_PARAM_NAME_COL || e.ColumnIndex == FIELD_EDITOR_NAME_COL) &&
            ...
            if (e.ColumnIndex != FIELD_VALUE_COL || e.RowIndex < 0 || this.dgvRows.CurrentCell == null) return;
            var cell2 = this.dgvCells.Rows[e.RowIndex].DataBoundItem as ICell;
            var tooltip = "";
            string cell_name = cell2.Name;
            object cell_value = cell2.Value;

            if (cell2.GetCellReferences() is { } def_names) {
                ParamWrapper wrapper = this.primary_result.ParamWrappers.Find(wrapper => Array.Exists(def_names, x => x == wrapper.Name));
                var v = Convert.ToInt32(cell_value);
                if (cell_name == "behaviorVariationId")
                    tooltip = wrapper.Rows
                                     .Where(row => row["variationId"] is { } field && Convert.ToInt32(field.Value) == v)
                                     .Aggregate(tooltip, (current, row) => $"{current}{v} {row.Name ?? ""}\n");
                else if (wrapper.Rows.Find(x => x.ID == v) is { } row) tooltip = $"{tooltip}{v} {row.Name ?? ""}\n";
            }

            if (settings.ParamDifferenceMode && this.secondary_result is not null) {
                var current_param_name = this.dgvParams.CurrentCell.Value.ToString();
                tooltip = this.secondary_result.ParamWrappers
                              .Find(secondary_wrapper => secondary_wrapper.Name == current_param_name)[
                               (this.dgvRows.Rows[this.dgvRows.CurrentCell.RowIndex].DataBoundItem as Row).ID]
                              .Cells
                              .Where(secondary_cell => cell2.Def.DisplayName == secondary_cell.Def.DisplayName &&
                                                       cell2.Def.DisplayType != PARAMDEF.DefType.dummy8 && !cell2.Value.Equals(secondary_cell.Value))
                              .Aggregate(tooltip, static (current, secondary_cell) => current + $"Secondary Value: {secondary_cell.Value}\n");
            }
        ...
        private void dgvCells_CellMouseDown(object sender, DataGridViewCellMouseEventArgs e) {
            if (this.InvalidationMode || e.ColumnIndex == -1 || e.RowIndex == -1 || e.Button != MouseButtons.Right) return;
            ...
            if (currentCell?.ContextMenuStrip is not { } cms) return;
        ...
        private void dgvCells_CellContextMenuStripNeeded(object sender, DataGridViewCellContextMenuStripNeededEventArgs e) {
            ...
            if (e.RowIndex == -1 || e.ColumnIndex == -1) return;
            if (this.dgvCells.Rows[e.RowIndex].DataBoundItem is ICell cell && cell.GetCellReferences() != null && Convert.ToInt32(cell.Value) > -1)
        ...
        private unsafe void fieldContextMenu_Opening(object sender, CancelEventArgs e) {
            var cell = this.dgvCells.Rows[this.dgvCells.CurrentCell.RowIndex].DataBoundItem as ICell;
            var cell_value_id = Convert.ToInt32(cell.Value);
            var behaviorRow_FirstOnly = false;
            ...
            if (cell.GetCellReferences() is not { } def_names) return;
            bool isBehaviorVariationID = cell.Name is "behaviorVariationId" or "Behavior Variation ID";
            ...
            foreach (ParamWrapper wrapper in this.primary_result.ParamWrappers) {
                ...
                for (int i = 0, count = wrapper.Rows.Count; i < count; i++) {
                    ...
                    if (!behaviorRow_FirstOnly && isBehaviorVariationID) {
                        ICell* cellp = AsPointer<IReadOnlyList<ICell>, ICell>(row.Cells);

                        for (int i2 = 2, count2 = row.Cells.Count + 2; i2 < count2; i2++) {
                            ICell behavior_cell = cellp[i2];
                            if (behavior_cell.Def.InternalName != "variationId" || Convert.ToInt32(behavior_cell.Value) != cell_value_id) continue;
        ...
        private void GotoReferenceHelper(string paramref) {
            ...
            for (var i = 0; i < this.dgvParams.Rows.Count; i++) {
                var name = this.dgvParams.Rows[i].Cells[0].Value.ToString();
                if (paramref != name) continue;
                this.dgvParams.ClearSelection();
                this.dgvParams.Rows[i].Selected = true;
            }

            if (cell.Name is "behaviorVariationId" or "Behavior Variation ID" && cell.GetCellReferences() is { } def_names) {
                var target_row = 0;
                var isBehaviorMatched = false;

                foreach (ParamWrapper wrapper in this.primary_result.ParamWrappers.Where(wrapper => Array.Exists(def_names, x => x == wrapper.Name) &&
                                                                                             !isBehaviorMatched))
                    foreach (Row wrapper_row in wrapper.Rows.Where(_ => !isBehaviorMatched)) {
                        foreach (ICell wrapper_cell in wrapper_row.Cells) {
                            if (isBehaviorMatched || (wrapper_cell.Name != "variationId" && wrapper_cell.EditorName != "Variation ID") ||
                                cell_value_id != Convert.ToInt32(wrapper_cell.Value)) continue;
                            target_row        = wrapper_row.ID;
                            isBehaviorMatched = true;
                        }
                    }

                for (var j = 0; j < this.dgvRows.Rows.Count; j++)
                    if (target_row == Convert.ToInt32(this.dgvRows.Rows[j].Cells[0].Value)) {
                        this.dgvRows.ClearSelection();
                        this.dgvRows.Rows[j].Selected = true;
                        this.dgvRows.CurrentCell      = this.dgvRows.Rows[j].Cells[0];
                    }
        ...
        private void GotoReference1MenuItem_Click(object sender, EventArgs e) =>
            this.GotoReferenceHelper((this.dgvCells.Rows[this.dgvCells.CurrentCell.RowIndex].DataBoundItem as ICell).GetCellReferences()[0]);
        ...
        private void GotoReference2MenuItem_Click(object sender, EventArgs e) =>
            this.GotoReferenceHelper((this.dgvCells.Rows[this.dgvCells.CurrentCell.RowIndex].DataBoundItem as ICell).GetCellReferences()[1]);
        ...
        private void GotoReference3MenuItem_Click(object sender, EventArgs e) =>
            this.GotoReferenceHelper((this.dgvCells.Rows[this.dgvCells.CurrentCell.RowIndex].DataBoundItem as ICell).GetCellReferences()[2]);
        ...
        private void GotoReference4MenuItem_Click(object sender, EventArgs e) =>
            this.GotoReferenceHelper((this.dgvCells.Rows[this.dgvCells.CurrentCell.RowIndex].DataBoundItem as ICell).GetCellReferences()[3]);
        ...
        private void GotoReference5MenuItem_Click(object sender, EventArgs e) =>
            this.GotoReferenceHelper((this.dgvCells.Rows[this.dgvCells.CurrentCell.RowIndex].DataBoundItem as ICell).GetCellReferences()[4]);
        ...
        private void GotoReference6MenuItem_Click(object sender, EventArgs e) =>
            this.GotoReferenceHelper((this.dgvCells.Rows[this.dgvCells.CurrentCell.RowIndex].DataBoundItem as ICell).GetCellReferences()[5]);
        ...
        private void selectSecondaryFileToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (this.secondaryFilePath.ShowDialog() != DialogResult.OK) return;
        ...
        private void clearSecondaryFileToolMenuItem_Click(object sender, EventArgs e) {
            ...
            settings.SecondaryFilePath      = this.secondaryFilePath.FileName;
            settings.ParamDifferenceMode    = false;
            if (!settings.ShowConfirmationMessages) _ = MessageBox.Show("Removed set secondary file path.", "Secondary File", MessageBoxButtons.OK);
        }
        ...
        private void button_FilterParams_Click(object sender, EventArgs e) {
            ...
            if (this.dgvParams.Rows.Count == 0) return;

            if (this.filter_Params.Text.ToLower().Split(section_delimiter) is { } input_list && input_list[0] != "") {
                this.dgvParamsParamCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
                this.EnterInvalidationMode();
                var hasSelectedParam = false;
                if (this.dgvRows.Rows.Count != 0) this.dgvRows.Rows[0].Selected   = true;
                if (this.dgvCells.Rows.Count != 0) this.dgvCells.Rows[0].Selected = true;
        ...
        private void button_ResetFilterParams_Click(object sender, EventArgs e) {
            this.dgvParamsParamCol.AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
            var hasSelectedFirstMatch = false;
            ...
            for (var i = 0; i < this.dgvParams.Rows.Count; i++) {
                ...
                if (hasSelectedFirstMatch) continue;
        ...
        private void button_FilterRows_Click(object sender, EventArgs e) {
            ...
            char[] command_delimiter = command_delimiter_string.ToCharArray();
            char[] section_delimiter = settings.Filter_SectionDelimiter.ToCharArray();
            if (this.dgvRows.Rows.Count == 0) return;

            if (this.filter_Rows.Text.ToLower().Split(section_delimiter) is { } input_list && input_list[0] != "") {
                ...
                foreach (DataGridViewRow current_row in this.dgvRows.Rows)
                    current_row.Selected = (current_row.Visible =
                            ...
                            current_row.Cells[this.ROW_ID_COL].Value.ToString().ToLower() is { } current_row_id &&
                            current_row.Cells[ROW_NAME_COL].Value.ToString().ToLower() is { } current_row_name &&
                            input_list.All(current_input => current_input != "" && (
                                               current_input.Contains($"view{command_delimiter_string}")
                                                   ? BuildViewList("Views\\\\Row\\\\",
                                                                   current_input.Split(command_delimiter)[1].TrimStart(' ').ToLower())
                                                       .Any(view_name => current_row_name.Contains(view_name) ||
                                                                         current_row_id.Contains(view_name))
                                                   : current_input.Contains($"exact{command_delimiter_string}")
                                                       ...
                                                       : current_input.Contains($"field{command_delimiter_string}") &&
                                                         current_input.Split(command_delimiter) is { } temp_input &&
                                                         temp_input[1].TrimStart(' ').ToLower() is { } field_input &&
                                                         field_input != "" &&
                                                         temp_input[2].TrimStart(' ').ToLower() is { } value_input &&
                                                         value_input != ""
                                                           ? row_data.Cells.Any(cell =>
                                                                                    (cell.EditorName.ToString().ToLower() == field_input ||
                                                                                     cell.Name.ToString().ToLower() == field_input) &&
                                                                                    cell.Value.ToString() is { } field_value &&
                                                                                    ((value_input.Contains(">=") &&
                                                                                      Convert.ToSingle(field_value) >=
                                                                                      Convert.ToSingle(value_input.Replace(">=", ""))) ||
                                                                                     (value_input.Contains('>') &&
                                                                                      Convert.ToSingle(field_value) >
                                                                                      Convert.ToSingle(value_input.Replace(">", ""))) ||
                                                                                     (value_input.Contains("<=") &&
        ...
        private void button_ResetFilterRows_Click(object sender, EventArgs e) {
            ...
            for (var i = 0; i < this.dgvRows.Rows.Count; i++) {
        ...
        private void ApplyCellFilter(bool invokeInvalidationMode) {
            ...
            char[] section_delimiter = settings.Filter_SectionDelimiter.ToCharArray();
            string command_delimiter_string = settings.Filter_CommandDelimiter;
            if (this.dgvCells.Rows.Count == 0) return;
            if (this.filter_Cells.Text == "") return;
            ...
            if (invokeInvalidationMode) this.EnterInvalidationMode();
            ...
            if (input_list[0].Length > 0) {
                ...
                for (var i = 0; i < this.dgvCells.Rows.Count; i++) {
                    ...
                    if (input_list.All([MethodImpl(MethodImplOptions.AggressiveInlining)](current_input) =>
                                           (current_input.Contains($"view{command_delimiter_string}")
                                            && BuildViewList("Views\\\\Field\\\\", current_input.Split(command_delimiter)[1].TrimStart(' ').ToLower())
                                                .Any(view_name => cell_row_param_name.Contains(view_name) ||
                                                                  cell_row_editor_name.Contains(view_name) || cell_row_value.Contains(view_name)))
                                           || (current_input.Contains($"exact{command_delimiter_string}")
                                               && (current_input = current_input.Split(command_delimiter)[1].TrimStart(' ').ToLower()).Length != 0
                                               && (cell_row_param_name.Equals(current_input) || cell_row_editor_name.Equals(current_input) ||
                                                   cell_row_value.Equals(current_input)))
                                           || (current_input.Length > 0 && (cell_row_param_name.Contains(current_input) ||
                                                                            cell_row_editor_name.Contains(current_input) ||
                                                                            cell_row_value.Contains(current_input))))) {
            ...
            if (invokeInvalidationMode) this.ExitInvalidationMode();
        }
        ...
        private void GenerateProjectDirectories(string project) {
            ...
            if (!flag) _ = Directory.CreateDirectory(projectDir);

            foreach (string folder in folders) {
                foreach (string type in gametypes) _ = Directory.CreateDirectory($"{projectDir}/{folder}/{type}");
            }
        }
        ...
        private void MakeDarkTheme(bool theme = true) {
            ...
            if (theme) {
                ...
                foreach (DataGridViewRow row in this.dgvCells.Rows) row.Cells[1].Style.BackColor = ControlsBackColor;
        ...
        public readonly struct PointerKeeper<T> {
            ...
            public unsafe PointerKeeper(T* p) => this._value = AsManaged<nuint>(p);

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public unsafe PointerKeeper(ref T p) => this._value = (nuint)AsPointer(ref p);
        ...
        public class LoadParamsResult {
            ...
            [Bindable(true)] public List<ParamWrapper> ParamWrappers;
        }
    }
}
