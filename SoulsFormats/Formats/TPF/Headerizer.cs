...
using static SoulsFormats.Formats.TPF.DDS;
...
namespace SoulsFormats.Formats.TPF {
    ...
    */
    internal static class Headerizer {
        ...
        public static byte[] Headerize(TPF.Texture texture) {
            ...
            if (DX10Formats.Contains(format)) {
                dds.header10 = new HEADER_DXT10 {
                    dxgiFormat = (DXGI_FORMAT)texture.Header.DXGIFormat
                };
                if (type == TPF.TexType.Cubemap) {
        ...
        private static int DetermineMipCount(int width, int height) => (int)Math.Ceiling(Math.Log(Math.Max(width, height), 2)) + 1;
        ...
        private static byte[] RebuildPixelData(byte[] bytes, byte format, short width, short height, int mipCount, TPF.TexType type) {
            ...
            List<Image> images = CompressedBPB.ContainsKey(format)
                ? Image.ReadCompressed(bytes, width, height, padDimensions, imageCount, mipCount, 0x80, CompressedBPB[format])
                : UncompressedBPP.ContainsKey(format)
                ? Image.ReadUncompressed(bytes, width, height, padDimensions, imageCount, mipCount, 0x80, UncompressedBPP[format])
                : throw new NotSupportedException($"Cannot decompose format {format}.");
            if (format is 10 or 102) {
                int texelSize = -1;
                if (format == 10) {
                    texelSize = 4;
                } else if (format == 102) {
        ...
        private static int PadTo(int value, int pad) => (int)Math.Ceiling(value / (float)pad) * pad;
        ...
        private class Image {
            ...
            public Image() => this.MipLevels = new List<byte[]>();
