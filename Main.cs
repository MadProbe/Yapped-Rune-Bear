...
namespace Chomp {
    ...
    public partial class Main : Form {
        ...
        public Main() {
            ...
            this.GoToReferenceMenuStripItems = new ToolStripMenuItem[6] {
        ...
        private void Main_Load(object sender, EventArgs e) {
            this.Text = "Yapped - Rune Bear Edition";
            ...
            _ = typeof(DataGridView).InvokeMember("DoubleBuffered", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.SetProperty, null, this.dgvParams, new object[] { true });
            _ = typeof(DataGridView).InvokeMember("DoubleBuffered", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.SetProperty, null, this.dgvRows, new object[] { true });
            _ = typeof(DataGridView).InvokeMember("DoubleBuffered", BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.SetProperty, null, this.dgvCells, new object[] { true });
            ...
            GameMode.GameType game = System.Enum.Parse<GameMode.GameType>(settings.GameType);
        ...
        private void LoadParams(bool isSilent) {
            ...
            // foreach (DataGridViewCell cell in this.dgvParams.Rows.Cast<DataGridViewRow>().SelectMany(x => x.Cells.Cast<DataGridViewCell>())) {
            //     cell.Value = (cell.OwningRow.DataBoundItem as ParamWrapper).Name;
            // }
        ...
        private void BuildParamDefs() {
            ...
            foreach (string path in Directory.GetFiles(this.GetParamdexDirectory("Defs"), "*.xml")) {
                try {
                    ...
                    Utility.ShowError($"Failed to load layout {Path.GetFileNameWithoutExtension(path)}.txt\r\n\r\n{ex}");
                }
            }
        }
        ...
        private void BuildParamTdfs() {
            ...
            foreach (string text in Directory.GetFiles(this.GetParamdexDirectory("Tdfs"), "*.tdf")) {
                try {
                    ...
                    Utility.ShowError($"Failed to load layout {Path.GetFileNameWithoutExtension(text)}.txt\r\n\r\n{ex}");
                }
            }
            foreach (PARAMTDF paramtdf2 in this.paramtdfs) {
                try {
                    ...
                    Utility.ShowError($"Failed to add TDF {paramtdf2.Name}.\r\n\r\n{ex2}");
                }
            }
        }
        ...
        private void SaveParams(string backup_format) {
            foreach (BinderFile file in this.regulation.Files) {
                foreach (DataGridViewRow obj in this.dgvParams.Rows) {
                    ParamWrapper paramFile = obj.DataBoundItem as ParamWrapper ?? this.ThrowCannotGetParamWrapper();
        ...
        private void DuplicateRowToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (newRowForm.ShowDialog() == DialogResult.OK) {
                ...
                ParamWrapper paramWrapper = this.rowSource.DataSource as ParamWrapper ?? this.ThrowCannotGetParamWrapper();
                Row newRow;
        ...
        private Row CreateRow(string prompt) {
            ...
            if (newRowForm.ShowDialog() == DialogResult.OK) {
                ...
                if (paramWrapper.Rows.Any(row => row.ID == id)) {
                    Utility.ShowError(string.Format("A row with this ID already exists: {0}", id));
                } else {
        ...
        private void GotoRowToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (gotoForm.ShowDialog() == DialogResult.OK) {
                ...
                Utility.ShowError(string.Format("Row ID not found: {0}", id));
            }
        }
        ...
        private void massImportDataMenuItem_Click(object sender, EventArgs e) => _ = !this.InvalidationMode
                && MessageBox.Show("Mass Import will import data from CSV files for all params. Continue?", "Mass Import", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation) != DialogResult.No
                ...
                })} ms!", "Mass Import") > DialogResult.None;
        ...
        private void massExportDataMenuItem_Click(object sender, EventArgs e) => _ = this.InvalidationMode ||
                ...
                || this.primary_result != null && MessageBox.Show($"Mass data export complete in {Globals.MeasureTimeSpent(() => this.primary_result.ParamWrappers.ForEach(wrapper => this.ExportParamData(wrapper, true)))} ms!", "Data Export") > DialogResult.None;
        ...
        [DoesNotReturn]
        private ParamWrapper ThrowCannotGetParamWrapper() => throw new NullReferenceException($"{nameof(this.rowSource.DataSource)} Row source has null DataSource");
        ...
        private unsafe void ImportParamData(ParamWrapper wrapper, bool isSilent) {
            ...
            try {
                ...
                Utility.ShowError($"Failed to open {paramPath}.\r\n\r\n{ex}");
                return;
            }
            _ = reader.ReadLine();
            List<Row> rows = wrapper.Rows;
            Row* rowp__temp = rows.AsPointer();
            ref Row* rowp = ref rowp__temp;
            int cells_length = (rows[0].Cells as Cell[]).Length + 2;
            int rows_length = rows.Count;
            ...
            fixed (char* export_delimeter_p = export_delimeter) {
                while (!reader.EndOfStream) {
                    ...
                    var splitter = new IterativeStringSplitter(line, export_delimeter_p, export_delimeter_length);
                    ...
                    if (rows_length > i) {
                        ...
                        newRow = *rowp++;
                        newRow.ID = id;
                        ReadOnlySpan<char> chars__ = splitter.Next();
                        int chars__length = chars__.Length;
                        if (includeRowNameInCSV && chars__length != 0) {
                            if (true || newRow.Name.Length <= chars__length) {
                                newRow.Name = new string(chars__);
                            } else {
                                string str = FastAllocateString(chars__length);
                                ulong* namep = AsPointer<string, ulong>(newRow.Name, 12); // start of string contents (+ string length)
                                ulong* charsp = chars__.AsPointer<char, ulong>();
                                int j = 0, l = chars__length >> 2;
                                for (; j < l; j++) {
                                    namep[j] = charsp[j];
                                }
                                if ((chars__length & 2) != 0) {
                                    const int mask = -0x7ffffffc;
                                    *PointerOffset<ulong, uint>(namep, (chars__length & mask) << 1) = *PointerOffset<ulong, uint>(charsp, (chars__length & mask) << 1);
                                }
                                if ((chars__length & 1) != 0) {
                                    const int mask = -0x7ffffffe;
                                    *PointerOffset<ulong, char>(namep, (chars__length & mask) << 1) = *PointerOffset<ulong, char>(charsp, (chars__length & mask) << 1);
                                }
                                //const int _ = unchecked((int)0xffffff00);
                                //*AsPointer<string, char>(newRow.Name, (chars__length & _) << 1) = NullCharacter;
                            }
                        } else {
                            newRow.Name = string.Empty;
                        }
                    } else {
                        newRow = rows.AppendValue(new Row(id, includeRowNameInCSV ? new string(splitter.Next()) : string.Empty, wrapper.AppliedParamDef));
                    }
                    ...
                    {
                        int i2 = 2;
                        for (Cell* cellp = AsPointer<IReadOnlyList<Cell>, Cell>(newRow.Cells); i2 != cells_length; i2++) {
                            Cell cell = cellp[i2];
                            if (cell.Def.DisplayType != PARAMDEF.DefType.dummy8) {
                                ...
                                try {
                                    ...
                                    _ = MessageBox.Show($"Row {newRow.ID}, Field {cell.Name} of {cell.Type} and {cell.EditorName} has invalid value \"{chars}\", skipped import of this value.", "Data Import");
                                }
                            }
                        }
                    }
            ...
            if (i >= rows_length) {
                if (i != rows_length) {
                    *AsPointer<List<Row>, int>(rows, 40) = i; // Set List size;
                                                              // Clear references to remaining rows so they can be later reclaimed by GC
                    for (ulong* row = (ulong*)rowp + i, end = (ulong*)rowp + rows_length; row != end; row++) {
                        *row = 0;
                    }
                }
            } else {
                wrapper.Rows.Sort((Row r1, Row r2) => r1.ID - r2.ID);
            }
            reader.Close();
        ...
        private unsafe void ExportParamData(ParamWrapper wrapper, bool isSilent) {
            ...
            ReadOnlySpan<char> new_line_chars = Environment.NewLine;
            ...
            using (var output_file = new StreamWriter(paramPath, new FileStreamOptions {
                ...
                if (verboseCSVExport) {
                    ...
                    int length;
                    var cells__ = rows[0].Cells as Cell[];
                    for (length = cells__.Length; length >= 0 && cells__[length - 1].Def.DisplayType == PARAMDEF.DefType.dummy8;) {
                        length--;
                    }
                    if (settings.IncludeHeaderInCSV) {
                        ...
                        for (int i = 0; i < length;) {
                            ...
                            if (cell.Def.DisplayType != PARAMDEF.DefType.dummy8) {
                                if (length == ++i) {
                                    output_file.WriteLine(cell.Def.InternalName);
                                } else {
                                    output_file.Write(cell.Def.InternalName);
                    ...
                    for (Row* row = rows.AsPointer(), l = row + rows.Count; row != l; row++) { // C++ style iteration xD (one of the most efficient means of array iteration btw)
                         ...
                         * struct C#Array {
                         *     void* objectHandle; // offset: 0
                         *     ulong length; // offset: 8
                         *     ...void* data; // offset: 16, end: length * sizeof(void*) + 8
                         * }
                         */
                        for (Cell* cell = AsPointer<IReadOnlyList<Cell>, Cell>(row->Cells, 16), end = cell + length, end2 = end - 1; cell != end; cell++) {
                            if (cell->Def.DisplayType != PARAMDEF.DefType.dummy8) {
                                if (end2 == cell) {
                                    output_file.WriteLine(cell->Value.ToString());
                                } else {
                                    output_file.Write(cell->Value.ToString());
        ...
        private void DgvParams_SelectionChanged(object sender, EventArgs e) {
            if (this.rowSource.DataSource != null) {
                this.dgvIndices[(this.rowSource.DataSource as ParamWrapper ?? this.ThrowCannotGetParamWrapper()).Name] = (
            ...
            if (this.dgvParams.SelectedCells.Count > 0) {
                settings.SelectedParam = this.dgvParams.SelectedCells[0].RowIndex;
                ParamWrapper paramFile2 = this.dgvParams.SelectedCells[0].OwningRow.DataBoundItem as ParamWrapper ?? this.ThrowCannotGetParamWrapper();
        ...
        private void DgvParams_CellToolTipTextNeeded(object sender, DataGridViewCellToolTipTextNeededEventArgs e) {
            if (e.RowIndex >= 0) {
                ParamWrapper paramWrapper = this.dgvParams.Rows[e.RowIndex].DataBoundItem as ParamWrapper ?? this.ThrowCannotGetParamWrapper();
        ...
        private void DgvRows_SelectionChanged(object sender, EventArgs e) {
            if (this.dgvRows.SelectedCells.Count > 0) {
                ...
                ParamWrapper paramFile = this.dgvParams.SelectedCells[0].OwningRow.DataBoundItem as ParamWrapper ?? this.ThrowCannotGetParamWrapper();
        ...
        private unsafe void fieldContextMenu_Opening(object sender, CancelEventArgs e) {
            ...
            if (cell.GetCellReferences() is string[] def_names) {
                ...
                foreach (ParamWrapper wrapper in this.primary_result.ParamWrappers) {
                    ...
                    if (index != -1) {
                        Row* rowp = wrapper.Rows.AsPointer();
        ...
        private void MakeDarkTheme(bool theme = true) {
            ...
            if (theme) {
                ...
                this.fileToolStripMenuItem.SetBackAndForeColors(BlackThemeTextColor, ControlsBackColor);
                this.addRowToolStripMenuItem.SetBackAndForeColors(BlackThemeTextColor, ControlsBackColor);
                this.ToolStripMenuItem.SetBackAndForeColors(BlackThemeTextColor, ControlsBackColor);
                this.editToolStripMenuItem.SetBackAndForeColors(BlackThemeTextColor, ControlsBackColor);
                this.secondaryDataToolStripMenuItem.SetBackAndForeColors(BlackThemeTextColor, ControlsBackColor);
                this.WorkflowToolStripMenuItem.SetBackAndForeColors(BlackThemeTextColor, ControlsBackColor);
                this.settingsMenuItem.SetBackAndForeColors(BlackThemeTextColor, ControlsBackColor);
                this.viewToolStripMenuItem.SetBackAndForeColors(BlackThemeTextColor, ControlsBackColor);
